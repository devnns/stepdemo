 ...
 */
class StepAccelerometerEventListener implements SensorEventListener{
    ...
    private int count = 0;
    private int mCurrentStep;//今日累计步数
    private MyStepCounterListener mMyStepCounterListener;
    private Context mContext;
    private long timeOfLastPeak1 = 0;
    private long timeOfThisPeak1 = 0;
    private String mStoredTodayDate;

    private BroadcastReceiver mBatInfoReceiver;

    public StepAccelerometerEventListener(Context context, MyStepCounterListener myStepCounterListener){
        super();
        mContext = context;
        ...
        mCurrentStep = (int) PreferencesHelper.getCurrentStep(mContext);//上次保存的步数，第一次是0

//        Log.i(TAG,"PreferencesHelper.getCurrentStep:"+ mCurrentStep);

        mStoredTodayDate = PreferencesHelper.getStoredTodayDate(mContext);//上一次记录时的日期，第一次是空

//        Log.i(TAG,"PreferencesHelper.getStepToday:"+ mStoredTodayDate);
    ...
    private synchronized void checkIfDateChanged() {
        //时间改变了清零，或者0点分隔回调
        if (!getSystemTodayDate().equals(mStoredTodayDate)) {
//
            WakeLockUtils.getLock(mContext);
            ...
            mStoredTodayDate = getSystemTodayDate();

            PreferencesHelper.setStoredTodayDate(mContext, mStoredTodayDate);
    ...
    private String getSystemTodayDate() {
